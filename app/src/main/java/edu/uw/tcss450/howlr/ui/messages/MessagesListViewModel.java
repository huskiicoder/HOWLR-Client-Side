...
public class MessagesListViewModel extends AndroidViewModel  {
    ...
    public void addMessagesListObserver(@NonNull LifecycleOwner owner,
                                    @NonNull Observer<? super List<ChatMessage>> observer) {
        mMessagesList.observe(owner, (Observer<? super List<MessageModel>>) observer);
    }
    ...
    private void handleResult(final JSONObject result) {
        ...
        try {
            ...
            if (root.has(getString.apply(R.string.keys_json_messages_response))) {
                JSONObject response =
                        root.getJSONObject(getString.apply(
                                R.string.keys_json_messages_response));
                if (response.has(getString.apply(R.string.keys_json_messages_data))) {
                    JSONArray data = response.getJSONArray(
                            getString.apply(R.string.keys_json_messages_data));

                    for(int i = 0; i < data.length(); i++) {
                        JSONObject jsonBlog = data.getJSONObject(i);
                        MessageModel cm = new MessageModel(
                                jsonBlog.getInt("chatid"),
                                jsonBlog.getString("firstname"),
                                jsonBlog.getString("message"),
                                jsonBlog.getString("timestamp"));
 /*                               jsonBlog.getString(
                                        getString.apply(
                                                R.string.keys_json_messages_chatID)),
                                jsonBlog.getString(
                                        getString.apply(
                                                R.string.keys_json_messages_recent_name)))
                                .addTeaser(jsonBlog.getString(
                                        getString.apply(
                                                R.string.keys_json_messages_recent_message)))
                                .addUrl(jsonBlog.getString(
                                        getString.apply(
                                                R.string.keys_json_messages_recent_timestamp)));*/
